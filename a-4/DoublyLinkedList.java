...
public class DoublyLinkedList {
	...
	private static class Node {
		 ...
		 * @param value_pass The vehicle
		 * @param p          The previous node
		 * @param n          The next node
		 */
		public Node(String key_pass, Vehicle value_pass, Node p, Node n) {
			key = key_pass;
			value = value_pass;
			prev = p;
			next = n;
	...
	/**
	 * 
	 * @return First vehicle in list
	 */
	public Vehicle firstValue() {
		if (isEmpty())
			return null;
	...
	/**
	 * 
	 * @return Last vehicle in list
	 */
	public Vehicle lastValue() {
		if (isEmpty())
			return null;
	 ...
	 * 
	 * @param key_pass   VIN
	 * @param value_pass Vehicle of VIN Adds this element to front of list
	 */
	public void addFirst(String key_pass, Vehicle value_pass) {
	 ...
	 * 
	 * @param key_pass   VIN
	 * @param value_pass Vehicle of VIN Adds this element to end of list
	 */
	public void addLast(String key_pass, Vehicle value_pass) {
	 ...
	 * @param value_pass  Vehicle to be added
	 * @param predecessor Previous node
	 * @param successor   Next node Adds this elements between nodes
	 */
	private void addBetween(String key_pass, Vehicle value_pass, Node pred, Node succ) {
		// Create a new node
	 ...
	 * @return The Vehicle with that key
	 */
	public Vehicle getValueFromKey(String key_pass) {
		Node current = header;
		...
		if (header == null && current.next == null) {
			...
			for (current = header.next; current.next != null; current = current.next) {
				...
				if (current.getKey().equals(key_pass)) {
					return (Vehicle) current.getValue();
				}
			}
			return null;
	...
	public void addHere(String k, Vehicle v) {
		Node current = header;
		for (current = header.next; current.next != null; current = current.next) {
